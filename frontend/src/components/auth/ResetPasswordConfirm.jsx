    ...
    try {
      ...
      try {
        ...
        if (isDevelopment && token === 'test-token') {
          console.warn('Mode développement: simulation de succès pour le token de test');
          setSuccess(true);
          setTimeout(() => {
            navigate('/login');
          }, 3000);
      ...
      <div className="mt-8 mb-4 bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
        ...
        {isDevelopment && token === 'test-token' && (
          <div className="bg-amber-100 border border-amber-400 text-amber-700 px-4 py-3 rounded mb-4" role="alert">
            <span className="block sm:inline font-medium">Mode développement</span>
            <span className="block sm:inline"> - Vous utilisez un token de test. La réinitialisation sera simulée.</span>
          </div>
        )}
